...
class hasil_pemeriksaan_bt_mp extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('M_lab/M_hasil_pemeriksaan/M_bahan_tambahan/M_hasil_pemeriksaan_bt_mp');
    ...
    {
        $data['result'] = $this->M_hasil_pemeriksaan_bt_mp->getmp()->result_array();
        ...
        $this->template->load('template', 'content/lab/hasil_pemeriksaan_bahan_tambahan/hasil_pemeriksaan_bt_mp.php', $data);
    }

    // Uji Bahan Tambahan (Methyl Paraben)
    public function add_ujimp()
    {
        ...
        $data['jarak_lebur'] = $this->input->post('jarak_lebur', TRUE);
        $data['keasaman'] = $this->input->post('keasaman', TRUE);
        $data['s_pengeringan'] = $this->input->post('s_pengeringan', TRUE);
        $data['s_pemijaran'] = $this->input->post('s_pemijaran', TRUE);
        $data['p_kadar'] = $this->input->post('p_kadar', TRUE);
        $data['kondisi_py'] = $this->input->post('kondisi_py', TRUE);

        $respon = $this->M_hasil_pemeriksaan_bt_mp->add_ujimp($data);

        $this->M_pemeriksaan_bahan->update_status_pb($data['id_adm_bm'], "Proses");

        if ($respon) {
            header('location:' . base_url('lab/pemeriksaan_bahan') . '?alert=success&msg=Selamat anda berhasil melakukan Uji Bahan Tambahan (Methyl Paraben)');
        } else {
            header('location:' . base_url('lab/pemeriksaan_bahan') . '?alert=error&msg=Maaf anda gagal melakukan Uji Bahan Tambahan (Methyl Paraben)');
        }
    }
    ...
    public function add()
    {
        ...
        $this->M_hasil_pemeriksaan_bt_mp->approval_rilis($data);
        $respon = $this->M_barang_masuk->add($data);

        if ($respon) {
            header('location:' . base_url('lab/Hasil_pemeriksaan_lab/Bahan_tambahan/Hasil_pemeriksaan_bt_mp') . '?alert=success&msg=Selamat anda berhasil menambah Barang Masuk Bahan Tambahan');
        } else {
            header('location:' . base_url('lab/Hasil_pemeriksaan_lab/Bahan_tambahan/Hasil_pemeriksaan_bt_mp') . '?alert=error&msg=Maaf anda gagal menambah Barang Masuk Bahan Tambahan');
        }
    }
    ...
    public function update()
    {
        ...
        $respon = $this->M_hasil_pemeriksaan_bt_mp->update($data);

        if ($respon) {
            header('location:' . base_url('lab/Hasil_pemeriksaan_lab/Bahan_tambahan/Hasil_pemeriksaan_bt_mp') . '?alert=success&msg=Selamat anda berhasil mengudate Bahan Tambahan (Methyl Paraben)');
        } else {
            header('location:' . base_url('lab/Hasil_pemeriksaan_lab/Bahan_tambahan/Hasil_pemeriksaan_bt_mp') . '?alert=error&msg=Maaf anda gagal mengupdate Bahan Tambahan (Methyl Paraben)');
        }
    }
    ...
    public function ditolak()
    {
        ...
        $respon = $this->M_hasil_pemeriksaan_bt_mp->approval_ditolak($data);

        if ($respon) {
            header('location:' . base_url('lab/Hasil_pemeriksaan_lab/Bahan_tambahan/Hasil_pemeriksaan_bt_mp') . '?alert=success&msg=Selamat anda berhasil Reject Bahan Tambahan');
        } else {
            header('location:' . base_url('lab/Hasil_pemeriksaan_lab/Bahan_tambahan/Hasil_pemeriksaan_bt_mp') . '?alert=error&msg=Maaf anda gagal Reject Bahan Tambahan');
        }
    }
    ...
    public function pdf_label_released($no_sj = null)
    {
        ...
        $data['detail'] = $this->M_hasil_pemeriksaan_bt_mp->ambil_label($no_surat_jalan)->result_array();

        $d = $this->load->view('laporan/hasil_pemeriksaan/hasil_pemeriksaan_bt/hasil_pemeriksaan_methyl_paraben/page_released', $data, TRUE);
    ...
    public function pdf_label_reject($no_sj = null)
    {
        ...
        $d = $this->load->view('laporan/hasil_pemeriksaan/hasil_pemeriksaan_bt/hasil_pemeriksaan_methyl_paraben/page_reject', $data, TRUE);
        $mpdf->AddPage("P", "", "", "", "", "15", "15", "5", "15", "", "", "", "", "", "", "", "", "", "", "", "A4");
        $mpdf->WriteHTML($d);
        $mpdf->Output();
    }
    ...
    public function pdf_label_hasil($no_sj = null)
    {
        ...
        $data['detail'] = $this->M_hasil_pemeriksaan_bt_mp->ambil_hasil($no_surat_jalan)->result_array();

        $d = $this->load->view('laporan/hasil_pemeriksaan/hasil_pemeriksaan_bt/hasil_pemeriksaan_methyl_paraben/page_hasil', $data, TRUE);
        $mpdf->AddPage("P", "", "", "", "", "15", "15", "5", "15", "", "", "", "", "", "", "", "", "", "", "", "A4");
        $mpdf->WriteHTML($d);
        $mpdf->Output();
    }
