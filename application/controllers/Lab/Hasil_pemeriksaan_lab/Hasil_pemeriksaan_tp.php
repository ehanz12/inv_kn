    ...
    private function convertDate($date)
{
    ...
    public function update()
    {
        ...
        $respon = $this->M_hasil_pemeriksaan_tp->update($data);

        if ($respon) {
            header('location:' . base_url('lab/Hasil_pemeriksaan_lab/Hasil_pemeriksaan_tp') . '?alert=success&msg=Selamat anda berhasil mengupdate Bahan Tinta Print');
        } else {
            header('location:' . base_url('lab/Hasil_pemeriksaan_lab/Hasil_pemeriksaan_tp') . '?alert=error&msg=Maaf anda gagal mengupdate Bahan Tinta Print');
        }
    }
