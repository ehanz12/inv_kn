...
class hasil_pemeriksaan_gel extends CI_Controller
{
    ...
    public function update()
    {
        ...
        $respon = $this->M_hasil_pemeriksaan_gel->update($data);

        if ($respon) {
            header('location:' . base_url('lab/Hasil_pemeriksaan_lab/Hasil_pemeriksaan_gel') . '?alert=success&msg=Selamat anda berhasil mengupdate Bahan Gelatin (Bahan Baku)');
        } else {
            header('location:' . base_url('lab/Hasil_pemeriksaan_lab/Hasil_pemeriksaan_gel') . '?alert=error&msg=Maaf anda gagal mengupdate Bahan Gelatin (Bahan Baku)');
        }
    }
