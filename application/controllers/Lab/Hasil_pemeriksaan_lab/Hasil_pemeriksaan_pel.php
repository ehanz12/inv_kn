...
class hasil_pemeriksaan_pel extends CI_Controller
{
    ...
    public function update()
    {
        $data['id_ujipel'] = $this->input->post('id_ujipel', TRUE);
        $data['id_adm_bm'] = $this->input->post('id_adm_bm', TRUE);
        $data['id_prc_master_barang'] = $this->input->post('id_bprc_master_barang', TRUE);
        ...
        $data['nama_operator'] = $this->input->post('nama_operator', TRUE);
        $data['pemerian'] = $this->input->post('pemerian', TRUE);
        $data['penguji'] = $this->input->post('penguji', TRUE);
        $data['aroma'] = $this->input->post('aroma', TRUE);
        $data['berat_jenis'] = $this->input->post('berat_jenis', TRUE);

        $respon = $this->M_hasil_pemeriksaan_pel->update($data);

        if ($respon) {
            header('location:' . base_url('lab/Hasil_pemeriksaan_lab/Hasil_pemeriksaan_pel') . '?alert=success&msg=Selamat anda berhasil mengupdate Bahan Pelarut');
        } else {
            header('location:' . base_url('lab/Hasil_pemeriksaan_lab/Hasil_pemeriksaan_pel') . '?alert=error&msg=Maaf anda gagal mengupdate Bahan Pelarut');
        }
    }
