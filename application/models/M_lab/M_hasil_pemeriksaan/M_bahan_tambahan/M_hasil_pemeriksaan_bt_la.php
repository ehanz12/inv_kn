...
class M_hasil_pemeriksaan_bt_la extends CI_Model
{
    ...
    public function add_ujila($data)
    {
        $id_user = $this->id_user();
        $sql = "
        INSERT INTO `tb_lab_hasil_ujibt_lecithinadlec`(`id_pb`,`id_barang`,`id_supplier`,`tgl_uji`,`no_analis`,`penguji`,`pemerian`,`kelarutan`,`viscosity`,`identifikasi`,`ang_lempeng_tot`,`kondisi_py`) 
        VALUES ('$data[id_pb]','$data[id_barang]','$data[id_supplier]','$data[tgl_uji]','$data[no_analis]','$data[nama_operator]','$data[pemerian]','$data[kelarutan]','$data[viscosity]','$data[identifikasi]','$data[ang_lempeng_tot]','$data[kondisi_py]')
        ";
        return $this->db->query($sql);
    }
    ...
    public function update($data)
    {
        $id_user = $this->id_user();
        $sql = "
        UPDATE `tb_lab_hasil_ujibt_lecithinadlec`
        SET 
        `id_pb` = '$data[id_pb]',
        ...
        `ang_lempeng_tot` = '$data[ang_lempeng_tot]',
    ...
    {
        $sql = "
        SELECT a.*,b.id_pb,b.no_batch,b.tgl,b.status,b.dok_pendukung,b.op_gudang,b.jenis_kemasan,b.jml_kemasan,b.jml_kemasan,b.qty,b.exp,b.mfg,b.tutup,b.wadah,b.label,c.nama_supplier,d.nama_barang,d.satuan,d.qty_pack FROM tb_lab_hasil_ujibt_lecithinadlec a
    ...
    {
        $sql = "
        SELECT a.*,b.id_pb,b.no_batch,b.tgl,b.status,b.dok_pendukung,b.op_gudang,b.jenis_kemasan,b.jml_kemasan,b.jml_kemasan,b.qty,b.exp,b.mfg,b.tutup,b.wadah,b.label,c.nama_supplier,d.nama_barang,d.satuan,d.nama_produsen,d.negara_produsen FROM tb_lab_hasil_ujibt_lecithinadlec a
        LEFT JOIN tb_lab_pemeriksaan_bahan b ON a.id_barang = b.id_barang
        LEFT JOIN tb_prc_supplier c ON a.id_supplier = c.id_supplier
        LEFT JOIN tb_prc_barang d ON a.id_barang = d.id_barang
        WHERE a.is_deleted = 0 AND b.no_surat_jalan = '$no_surat_jalan' ORDER BY a.tgl_uji ASC";
