...
class M_konfirmasi_pesanan extends CI_Model {
    ...
    // Get all data dengan subquery untuk menghitung outstanding
    public function get_all($nama_customer = null, $date_from = null, $date_until = null) {
        $sql = "
            SELECT 
                ...
                e.kode_warna_body,
                e.warna_body,
                -- Subquery untuk menghitung total produksi
                (SELECT COALESCE(SUM(jumlah_prd), 0) 
                 FROM tb_mkt_schedule 
                 WHERE id_mkt_kp = a.id_mkt_kp AND is_deleted = 0) as jumlah_prd,
                
                (a.jumlah_kp - (SELECT COALESCE(SUM(jumlah_prd), 0) 
                               FROM tb_mkt_schedule 
                               WHERE id_mkt_kp = a.id_mkt_kp AND is_deleted = 0)) as outstanding
            FROM tb_mkt_kp a
