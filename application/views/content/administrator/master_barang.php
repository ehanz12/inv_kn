            ...
            <div class="row">
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control text-uppercase" id="kode_barang" name="kode_barang" maxlength="20" placeholder="Kode Barang" aria-describedby="validationServer03Feedback" autocomplete="off" required>
  ...
  <script>
    $(document).ready(function() {
      ...
      $('#add').on('hidden.bs.modal', function() {
        ...
        // 2. Reset CHOSEN SELECT
        modal.find('.chosen-select')
          .val('')
          .trigger('chosen:updated');

        // 3. Hapus validasi error
        modal.find('.is-invalid').removeClass('is-invalid');

        // 4. Enable tombol simpan
        $('#simpan').prop('disabled', false);

        // 5. Kosongkan input manual
        modal.find('input[type="text"]').val('');

        // 6. Hide Mesh & Bloom + remove required
        $('#mesh_container, #bloom_container').hide();
        $('#mesh, #bloom').prop('required', false);
  ...
  < <!-- Modal Edit-->
              ...
              <div class="row">
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="e-kode_barang" name="kode_barang" placeholder="Kode Barang" aria-describedby="validationServer03Feedback" autocomplete="off" required>
                    <div id="validationServer03Feedback" class="invalid-feedback">
                      Maaf Kode Barang sudah ada.
                    </div>
                  </div>
    ...
    <script type="text/javascript">
      $(document).ready(function() {
        ...
        $("#e-kode_barang").keyup(function() {
          this.value = this.value.toUpperCase();
          var kode_barang = $("#e-kode_barang").val();
          jQuery.ajax({
            url: "<?= base_url() ?>administrator/master_barang/cek_kode_barang/",
            dataType: 'text',
            type: "post",
            ...
            success: function(response) {
              if (response == "true") {
                $("#e-kode_barang").addClass("is-invalid");
                $("#simpan").attr("disabled", "disabled");
              } else {
                $("#e-kode_barang").removeClass("is-invalid");
                $("#simpan").removeAttr("disabled");
              }
            }
          });
